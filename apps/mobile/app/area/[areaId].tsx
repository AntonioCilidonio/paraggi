      ...
      <View className="gap-6">
        <View className="flex-row items-center gap-3 border-b border-border pb-4">
          ...
          <View className="flex-1">
            <Text className="text-2xl font-bold text-ink">{area?.name ?? "Dettaglio area"}</Text>
        ...
        {detail.data ? (
          <>
            <View className="flex-row justify-between border-y border-border py-4">
            ...
            <View className="gap-3">
              ...
              {detail.data.posts.map((post) => (
                <Pressable key={post.id} accessibilityRole="button" onPress={() => router.push(`/post/${post.id}`)} className="border-b border-border py-3">
            ...
            <View className="gap-3">
              ...
              {detail.data.comments.map((comment) => (
                <Pressable key={comment.id} accessibilityRole="button" onPress={() => router.push(`/post/${comment.post_id}`)} className="flex-row gap-3 border-b border-border py-3">
                  <Ionicons name="chatbubble-outline" size={19} color="#16808a" />
            ...
            <View className="gap-3">
              ...
              {detail.data.connections.map((connection) => (
                <Pressable key={connection.id} accessibilityRole="button" disabled={!connection.chat?.id} onPress={() => connection.chat?.id && router.push(`/chat/${connection.chat.id}`)} className="flex-row items-center gap-3 border-b border-border py-3">
                  <View className="h-10 w-10 items-center justify-center rounded-full bg-surface"><Ionicons name="person-outline" size={19} color="#16808a" /></View>
