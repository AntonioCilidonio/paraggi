...
describe("resolveNotificationRoute", () => {
  ...
  it("opens chats, requests and SOS destinations", () => {
    expect(resolveNotificationRoute({ type: "private_request" })).toBe("/chats");
  ...
  it("prefers an explicit deep link and has a safe fallback", () => {
    ...
    expect(resolveNotificationRoute({ deepLink: "paraggi://danger/alert-2" })).toBe("/danger/alert-2");
  ...
  it("rejects unsupported stored routes instead of opening an unmatched page", () => {
    expect(resolveNotificationRoute({ type: "private_request" }, "/route/che-non-esiste")).toBe("/chats");
    expect(resolveNotificationRoute({}, "https://example.com/notifica")).toBe("/feed");
  });
});
