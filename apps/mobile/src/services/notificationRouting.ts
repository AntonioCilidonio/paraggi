...
function normalizeDeepLink(value: string | null) {
  ...
  if (route.startsWith("paraggi://")) route = `/${route.slice("paraggi://".length)}`;
  route = route
    .replace(/^\/\(tabs\)\//, "/")
    .split(/[?#]/, 1)[0];

  const staticRoutes = new Set(["/feed", "/chats", "/history", "/profile", "/heatmap", "/notifications"]);
...
export function resolveNotificationRoute(data: NotificationData, storedDeepLink?: string | null) {
  const deepLink = normalizeDeepLink(stringValue(data.deepLink) ?? storedDeepLink ?? null);
